...
if( !function_exists('fwroute') ){
  function fwroute($path){

    return site_url() . '/firmware/' .ltrim($path, '/');
  }
}
...
if( !function_exists('fwgetController') ){
  function fwgetController(){

    global $wp;

    if(isset($wp->query_vars[FWROUTE])){

      $path = explode( '/', trim( $wp->query_vars[FWROUTE], '/' ) );
...
if( !function_exists('fwgetMethod') ){
  function fwgetMethod(){
...
if( !function_exists('fwparseUrl') ){
  function fwparseUrl(){

    global $wp;

    $url = explode( '/', trim( $wp->query_vars[FWROUTE], '/' ) );

    unset($url[array_search(fwgetController(), $url)]);

    foreach($url as $key=>$param)
      if($param == fwgetMethod())
        unset($url[$key]);
