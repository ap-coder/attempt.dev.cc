...
<div class="wrap">
<h1 class="wp-heading-inline">Manage Product Firmware</h1>

<hr />

<form method="GET" action="<?php echo admin_url(); ?>admin.php?page=firmware-products">
	<p class="search-box">
	...
	<table class="widefat fixed" cellspacing="0">
		<thead>
			<tr>
				...
				<th class="manage-column column-columnname sortable <?php echo get_sort_classes($slug); ?>" scope="col" data-state="false" data-col="<?php echo $column; ?>">
					<a href="<?php echo get_admin_url();?>admin.php?page=firmware-products&<?php echo fill_query_string(array('orderby'=>$slug, 'order'=>$direction))?>"><span><?php echo $column; ?></span><span class="sorting-indicator"></span></a>
				 </th>
				<?php endforeach;?>
		...
		<tbody>
			<?php foreach($results as $key=>$result):?>
				<tr <?php echo $key % 2 == 0 ? 'class="alternate"':''; ?> data-row="<?php if ( is_object($result) ) { echo $result->id; } ?>">
					<td class="column-columnname"><?php echo stripslashes($result->product_name) ?></td>
					<td>[firmware product_id="<?php echo $result->ID;?>"]</td>
					<td class="column-columnname">
	          <a style="color: #00a0d2;" href="<?php echo site_url( '/wp-admin/admin.php?page=firmware-products&product_id='.$result->ID, $scheme = null )?>">View</a>
					</td>
				</tr>
			<?php endforeach;?>
		</tbody>
